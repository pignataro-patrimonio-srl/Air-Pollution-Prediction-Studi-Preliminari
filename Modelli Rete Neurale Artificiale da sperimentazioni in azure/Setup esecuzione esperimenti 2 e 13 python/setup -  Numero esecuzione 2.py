...
dataprep_json = "{\"datastoreName\":\"airpollutionpr1955096294__uploadedresources\",\"dataPath\":\"3BB83_4c90b8112a674300acd78a4409023426_34349a2bfbc34716afcae97f11f85586.csv\",\"columnSeparator\":\",\",\"promoteHeader\":true,\"encoding\":\"UTF8\",\"ignoreNewlineInQuotes\":true,\"skipRows\":0,\"features\":[\"created_at\",\"entry_id\",\"PM1.0_CF_ATM_ug/m3\",\"PM2.5_CF_ATM_ug/m3\",\"UptimeMinutes\",\"ADC\",\"Temperature_F\",\"Humidity_%\",\"PM2.5_CF_1_ug/m3\",\"\"],\"label\":\"PM10.0_CF_ATM_ug/m3\"}"  # PLACEHOLDER
num_iterations = None # PLACEHOLDER
enable_subsampling = False # PLACEHOLDER
run_id = "%%RUN_ID%%"  # PLACEHOLDER
automl_settings = {'name':'AirPollutionPrediction','path':'./sample_projects/AirPollutionPrediction','subscription_id':'194e4ef4-8e27-42f0-b765-96c584b38a54','resource_group':'Pignataro-Patrimonio','workspace_name':'AirPollutionPrediction','region':'westeurope','iterations':100,'primary_metric':'accuracy','data_script':None,'compute_target':'PPAPP','task_type':'classification','validation_size':None,'n_cross_validations':5,'y_min':None,'y_max':None,'num_classes':None,'preprocess':True,'lag_length':0,'max_cores_per_iteration':-1,'max_concurrent_iterations':6,'iteration_timeout_minutes':60,'mem_in_mb':None,'enforce_time_on_windows':True,'experiment_timeout_minutes':60,'experiment_exit_score':None,'whitelist_models':None,'auto_blacklist':False,'blacklist_samples_reached':False,'exclude_nan_labels':True,'verbosity':20,'debug_log':'automl_errors.log','show_warnings':False,'model_explainability':False,'service_url':None,'sdk_url':None,'sdk_packages':None,'telemetry_verbosity':'INFO','send_telemetry':True,'spark_service':None,'metrics':None,'enable_ensembling':True,'ensemble_iterations':15,'enable_tf':False,'enable_cache':True,'enable_subsampling':False,'subsample_seed':None,'cost_mode':0,'metric_operation':'maximize','is_timeseries':False,'time_column_name':None,'max_horizon':None,'enable_onnx_compatible_models':False,'enable_voting_ensemble':True,'enable_stack_ensemble':True,'enable_stack_ensembling':False,'enable_early_stopping':False,'early_stopping_n_iters':10,'enable_feature_sweeping':False} # PLACEHOLDER for AutoMLSettings



def new_setup_run(parameters):
    print("Starting the setup.py script in worker....")
    ...
    runtoken = parameters["AML_TOKEN"]
    script_directory = parameters["SCRIPT_DIRECTORY"]
    os.environ["AZUREML_RUN_TOKEN"] = runtoken
    print("RUN_ID : ", run_id)
    print("AZUREML_ARM_SUBSCRIPTION : ",os.environ["AZUREML_ARM_SUBSCRIPTION"])
    print("AZUREML_ARM_PROJECT_NAME : ",os.environ["AZUREML_ARM_PROJECT_NAME"])
